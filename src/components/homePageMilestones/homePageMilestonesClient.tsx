...
// ── All static data at module level — never recreated on re-render ──
...
// Pre-compute doubled array once at module level
const doubledTestimonials = [...testimonials, ...testimonials];

// Pre-compute logo sources for testimonials
const testimonialLogoSrcs = doubledTestimonials.map((t) =>
  getLogoSrc(t.company, getCleanDomain(t.domain))
);

// Pre-compute logo sources for footer
const footerLogoSrcs = footerCompanies.map((c) =>
  getLogoSrc(c.name, getCleanDomain(c.domain))
);

// Stable onError handler factory — avoids inline arrow per card
function makeLogoErrorHandler(companyName: string, cleanDomain: string) {
  return (e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    if (
      attempts <= 1 &&
      (target.src.includes("simpleicons.org") || target.src.includes("clearbit.com"))
    ) {
...
// Pre-build error handlers so they're stable references
const testimonialErrorHandlers = doubledTestimonials.map((t) =>
  makeLogoErrorHandler(t.company, getCleanDomain(t.domain))
);
const footerErrorHandlers = footerCompanies.map((c) =>
  makeLogoErrorHandler(c.name, getCleanDomain(c.domain))
);

function HomePageMilestonesClient() {
  return (
              ...
              <div className={styles.companyHeader}>
                <div className={styles.companyLogoIcon}>
                  <Image
                    ...
                    unoptimized
                    loading="lazy"
                    onError={testimonialErrorHandlers[i]}
                  />
                </div>
...
export default memo(HomePageMilestonesClient);
