...
import { Sparkles, Copy, Check, RotateCcw, AlignLeft, Loader2 } from "lucide-react";
...
export default function AiResumeSummaryGenerator() {
  ...
  const fetchSummaries = async (overrideData?: { role: string; years: string; skills: string; achievement: string; tone: string }) => {
    const payload = overrideData || { role, years, skills, achievement, tone };
    if (!payload.role.trim()) return;
    setLoading(true);
    setError("");
    try {
  ...
  const handleGenerate = () => fetchSummaries();
  ...
  const handleReset = () => {
    setRole(""); setYears(""); setSkills(""); setAchievement(""); setSummaries([]); setTone("Professional"); setError("");
  };
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 py-10 max-[768px]:py-8 max-[480px]:px-4">
        ...
        <div className="flex flex-col gap-2 max-w-2xl">
          <span className="inline-flex w-fit items-center gap-2 rounded-full border border-[#f0ded4] bg-white px-3 py-1 text-sm font-medium text-[#f55d1d]">
            <Sparkles size={14} /> AI Summary Generator
          </span>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          <div className="border border-[#f0ded4] bg-white p-7 shadow-[0_18px_60px_rgba(245,93,29,0.08)] rounded-2xl">
            ...
            <div className="flex flex-col gap-5">
              ...
              <button onClick={handleGenerate} disabled={!role.trim() || loading}
                ...
                {loading ? <><Loader2 size={15} className="animate-spin" /> Generating...</> : <><Sparkles size={15} /> Generate Summary</>}
              </button>
            </div>
          </div>
          ...
          {summaries.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-[#ead8cf] bg-white p-16 text-center max-[768px]:hidden">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#fff2ec]">
                {loading ? <Loader2 size={28} className="text-[#f55d1d] animate-spin" /> : <AlignLeft size={28} className="text-[#f55d1d]" />}
              </div>
              <p className="text-base font-bold text-[#312925]">{loading ? "Generating your summaries..." : "Your summaries will appear here"}</p>
              <p className="text-sm text-[#9c8880]">{loading ? "AI is crafting 3 tailored options" : "Fill in the form to generate 3 options"}</p>
            </div>
          )}
        </div>
      </div>
